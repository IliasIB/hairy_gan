...
def train_reconstruction(batch_size, iteration_amount, epoch_amount):
    ...
    recognizer_encoded, _ = recognizer(discriminator_encoded_layer, 64)

    # Real
    _, discriminator_real, discriminator_real_layer = discriminator(image)
    recognizer_real, _ = recognizer(discriminator_real_layer, 64)
    ...
    recognizer_random, _ = recognizer(discriminator_random_layer, 64)
...
def get_encoder_loss(decoder_encoded, image):
    # Encoder loss
    encode_decode_loss = tf.losses.mean_squared_error(labels=image, predictions=decoder_encoded)
